-- Migration: Add support for processed images (FIXED VERSION)
-- This migration adds columns and functions to support advanced image processing
-- FIXES: Corrected RLS policies to match actual database schema
...
-- FIXED RLS POLICIES FOR PROCESSED_IMAGES TABLE
-- These policies are corrected to match the actual database schema
ALTER TABLE processed_images ENABLE ROW LEVEL SECURITY;

-- Policy: Maids can view and manage their own processed images
CREATE POLICY "Maids can manage own processed images" ON processed_images
    FOR ALL USING (
        maid_profile_id = auth.uid()
    );

-- Policy: Sponsors can view processed images for maids who applied to their jobs
CREATE POLICY "Sponsors can view processed images for applicants" ON processed_images
    FOR SELECT USING (
        maid_profile_id IN (
            SELECT maid_id FROM applications 
            WHERE job_id IN (
                SELECT id FROM jobs WHERE sponsor_id = auth.uid()
            )
        )
    );

-- Policy: Agencies can view processed images for all maids (if they have agency role)
CREATE POLICY "Agencies can view all processed images" ON processed_images
    FOR SELECT USING (
        EXISTS (
...
-- Policy: Admins can manage all processed images
CREATE POLICY "Admins can manage all processed images" ON processed_images
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE id = auth.uid() 
...
-- Policy: Public can view processed images for available maids (for browsing)
CREATE POLICY "Public can view processed images for available maids" ON processed_images
    FOR SELECT USING (
        maid_profile_id IN (
...
-- Success message
SELECT 'Processed images migration completed successfully with fixed RLS policies!' as status;
