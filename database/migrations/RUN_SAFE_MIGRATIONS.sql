-- =============================================
-- ALL-IN-ONE SAFE MIGRATION RUNNER
-- Applies migrations 037-042 safely (can be re-run)
-- =============================================

-- This file consolidates all remaining migrations into one safe script
...
RAISE NOTICE '✓ Migration 037: Subscriptions table ready';
...
RAISE NOTICE '✓ Migration 038: Phone verifications table ready';

-- ============================================
-- MIGRATION 039: Add phone to profiles (SKIP if already has columns)
-- ============================================

-- This will be skipped if columns already exist
DO $$
BEGIN
  -- Add to sponsor_profiles
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='sponsor_profiles' AND column_name='phone_number') THEN
    ...
    RAISE NOTICE '✓ Added phone fields to sponsor_profiles';
  ELSE
    RAISE NOTICE '⏭️  Phone fields already exist in sponsor_profiles';
  END IF;

  -- Add to maid_profiles
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='maid_profiles' AND column_name='phone_number') THEN
    ...
    RAISE NOTICE '✓ Added phone fields to maid_profiles';
  ELSE
    RAISE NOTICE '⏭️  Phone fields already exist in maid_profiles';
  END IF;
END $$;

RAISE NOTICE '✓ Migration 039: Phone fields added to profiles';
...
RAISE NOTICE '✓ Migration 040: Two-factor backup codes table ready';
...
RAISE NOTICE '✓ Migration 041: Activity log table ready';
...
RAISE NOTICE '✓ Migration 042: Payment methods table ready';
...
BEGIN
  ...
  IF table_count = 5 THEN
    RAISE NOTICE '✅ SUCCESS: All 5 tables are ready!';
  ELSE
