...
-- Runs in correct order: Tables → Columns → Indexes → Policies
-- =============================================
...
-- STEP 1: Create basic table or ensure it exists
-- ============================================
...
-- STEP 2: Add primary key if not exists
DO $$
BEGIN
...
-- STEP 3: Add all missing columns ONE BY ONE
DO $$
BEGIN
...
-- STEP 4: Now create indexes (columns definitely exist now)
...
BEGIN
  ...
  WHERE table_name = 'activity_log'
    AND column_name IN ('action_type', 'entity_type', 'entity_id', 'details', 'metadata', 'ip_address', 'user_agent');

  RAISE NOTICE '========================================';
  RAISE NOTICE '✓ All migrations completed!';
  RAISE NOTICE 'Tables: % of 5', table_count;
  RAISE NOTICE 'Activity log columns: % of 7', column_count;
  RAISE NOTICE '========================================';

  IF table_count = 5 AND column_count = 7 THEN
    RAISE NOTICE '✅ SUCCESS: Everything is ready!';
  ELSE
    RAISE NOTICE 'Tables found: %', table_count;
    RAISE NOTICE 'Activity log columns found: %', column_count;
  END IF;
END $$;
