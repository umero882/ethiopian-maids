...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  ...
  console.error(
    '   VITE_SUPABASE_URL:',
    SUPABASE_URL ? 'âœ… Set' : 'âŒ Missing'
  );
  console.error(
    '   SUPABASE_SERVICE_KEY:',
    SUPABASE_SERVICE_KEY ? 'âœ… Set' : 'âŒ Missing'
  );
...
async function checkMigrationFile(filePath, fileName) {
  ...
  try {
    ...
    console.error(`âŒ Error reading ${fileName}:`, error.message);
    return false;
  }
}
...
async function checkMigrations() {
  ...
  console.log(
    'ðŸ“‹ Migration files to execute manually in Supabase SQL Editor:'
  );
  console.log(
    '================================================================\n'
  );
  ...
  for (const tableName of requiredTables) {
    try {
      ...
      if (error && error.code === '42P01') {
        console.log(`âŒ Table '${tableName}' missing`);
        allTablesExist = false;
      } else {
        console.log(`âœ… Table '${tableName}' exists`);
      }
    } catch (err) {
      console.log(`âŒ Error checking '${tableName}':`, err.message);
      allTablesExist = false;
    }
  }
...
async function main() {
  ...
  if (!connected) {
    console.error('âŒ Cannot proceed without database connection');
    process.exit(1);
  }
  ...
  if (!migrationsFound) {
    console.error('âŒ No migration files found');
    process.exit(1);
  }
