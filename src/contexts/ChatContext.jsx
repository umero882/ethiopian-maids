...
export const ChatProvider = ({ children, mockValue }) => {
  ...
  useEffect(() => {
    ...
    log.debug('Setting up online presence tracking');
    ...
    presenceChannel
      .on('presence', { event: 'sync' }, () => {
        ...
        log.debug('Online users synced:', users.length);
        setOnlineUsers(users);
      })
      .on('presence', { event: 'join' }, ({ key, newPresences }) => {
        log.debug('User joined:', key);
        setOnlineUsers((prev) => {
      ...
      .on('presence', { event: 'leave' }, ({ key, leftPresences }) => {
        log.debug('User left:', key);
        setOnlineUsers((prev) => prev.filter((u) => u.userId !== key));
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          log.info('Presence channel subscribed, tracking user:', user.id);
    ...
    return () => {
      log.debug('Cleaning up presence tracking');
  ...
  const value = useMemo(() => ({
    ...
    isUserOnline, // Expose online status checker
  }), [
    conversations,
    activeConversation,
    messages,
